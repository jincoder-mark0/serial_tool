...
- **예외 — 호출 시그니처가 바뀌어 실행 자체가 불가능해진 경우**에만 최소 변경으로 맞추고
  아래 "이후 조정" 절에 무엇을 왜 바꿨는지 적는다. 돌아가지 않는 스크립트는 근거가 아니다.
...
| 날짜 | 스크립트 | 무엇을 왜 |
|---|---|---|
| 2026-09-03 | `probe_right_panel_geometry.py`, `probe_layout_invalidation_strategies.py` | `MainWindow()`가 `MainWindow(theme_manager, color_manager)`로 바뀌어(View manager 싱글턴 해체) 실행 자체가 불가능해졌다. 생성 인자만 맞췄고 측정 로직은 손대지 않았다 |
