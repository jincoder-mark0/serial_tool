...
class TestSystemLogWidgetLineAppendedSignal:
    def test_append_log_emits_plain_text_without_html(self, qapp, qtbot):
        ...
        with qtbot.waitSignal(
            widget.system_log_line_appended, timeout=1000, raising=True
        ) as blocker:
    ...
    def test_filter_disabled_emits_every_line(self, qapp, qtbot):
        ...
        with qtbot.waitSignal(
            ...
            widget.append_log(
                SystemLogEvent(message="does not contain the word", level="INFO")
            )
    ...
    def test_filter_enabled_allows_matching_line(self, qapp, qtbot):
        ...
        with qtbot.waitSignal(
            ...
            widget.append_log(
                SystemLogEvent(message="this has TARGET inside", level="INFO")
            )
...
class TestSystemLogPersistenceIntegration:
    def test_start_line_stop_writes_and_closes_real_file(
        self, presenter_and_components, tmp_path
    ):
        presenter, components = presenter_and_components
        coordinator = components.logging_coordinator
        ...
        coordinator.on_system_log_line_appended(
            "[12:00:01] [INFO] should not be written"
        )
        assert file_path.read_text(encoding="utf-8") == "[12:00:00] [INFO] hello\n"
    ...
    def test_cancel_dialog_does_not_create_writer(self, presenter_and_components):
        ...
        components.logging_coordinator.on_system_logging_start_requested()
        assert components.logging_coordinator.system_log_writer is None
    ...
    def test_app_shutdown_closes_open_writer_without_data_loss(
        ...
        writer = coordinator.system_log_writer
        assert writer is not None and writer.is_open is True
