...
def _make_coordinator():
    ...
    return coordinator, runner, controller, service, port_view


def test_macro_start_snapshots_current_port_once():
    coordinator, _, _, _, port_view = _make_coordinator()
...
def test_repeated_worker_send_uses_snapshot_without_view_lookup():
    coordinator, _, _, service, port_view = _make_coordinator()
...
def test_single_send_reads_current_port_and_emits_local_echo():
    coordinator, _, _, service, port_view = _make_coordinator()
...
def test_single_send_failure_stops_and_surfaces_reason():
    coordinator, runner, _, service, _ = _make_coordinator()
...
def test_target_port_close_stops_non_broadcast_macro():
    coordinator, runner, _, _, _ = _make_coordinator()
    coordinator._target_port = "COM1"
    runner.broadcast_enabled = False
    reasons = []
    coordinator.execution_interrupted.connect(reasons.append)

    coordinator.on_connection_closed(
        PortConnectionEvent(port="COM1", state="closed")
    )
...
def test_other_port_close_does_not_stop_non_broadcast_macro():
    ...
    coordinator.on_connection_closed(
        PortConnectionEvent(port="COM2", state="closed")
    )
...
def test_broadcast_macro_stops_when_no_broadcast_ports_remain():
    coordinator, runner, controller, _, _ = _make_coordinator()
