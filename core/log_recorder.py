...
## WHAT
* LogRecorder: 단일 포트 녹화 담당
* LogRecorderManager: 여러 포트의 녹화 관리
...
class LogRecorder:
    """
    단일 포트의 실시간 로그 녹화를 담당합니다.
    
    백그라운드 스레드에서 파일 쓰기를 수행하여 UI에 영향을 주지 않습니다.
    Queue를 사용하여 데이터 누락을 방지합니다.
    
    Attributes:
        filepath (str): 녹화 파일 경로
        is_recording (bool): 녹화 중 여부
    """
...
class LogRecorderManager:
    ...
    def start_recording(self, port_name: str, filepath: str) -> bool:
        """
        특정 포트의 녹화를 시작합니다.
    ...
    def stop_recording(self, port_name: str) -> None:
        """
        특정 포트의 녹화를 중단합니다.
    ...
    def is_recording(self, port_name: str) -> bool:
        """
        특정 포트가 녹화 중인지 확인합니다.
        ...
        Returns:
            bool: 녹화 중이면 True
        """
        return port_name in self._recorders and self._recorders[port_name].is_recording
    
    def get_filepath(self, port_name: str) -> str:
        """
        특정 포트의 녹화 파일 경로를 반환합니다.
        
        Args:
            port_name: 포트 이름
            
        Returns:
            str: 파일 경로, 녹화 중이 아니면 빈 문자열
        """
        if port_name in self._recorders:
