...
class ConnectionController(QObject):
    ...
    특정 탭의 활성 상태를 저장하지 않으며, 메서드 호출 시 대상 포트 이름을 인자로 받습니다.
    """
    ...
    # -------------------------------------------------------------------------
    # File Transfer Lifecycle Management
    # -------------------------------------------------------------------------
    def register_file_transfer(self, port_name: str, file_transfer_service: 'FileTransferService') -> None:
        """
        파일 전송 시작 시 엔진을 등록합니다.
        포트가 닫힐 때 전송 중인 작업을 취소하기 위해 사용됩니다.

        Args:
            port_name (str): 포트 이름.
            file_transfer_service (FileTransferService): 실행 중인 전송 서비스 인스턴스.
        """
    ...
    def on_worker_closed(self, name: str) -> None:
        """
        Worker 스레드가 종료되었을 때 리소스를 정리합니다.

        Logic:
            - 관리 딕셔너리(workers, parsers, configs)에서 해당 포트 제거
            - PortConnectionEvent(closed) DTO 발행
    ...
    def close_connection(self, name: Optional[str] = None) -> None:
        ...
        Logic:
            - 특정 이름이 주어지면 해당 포트만 종료
            - 파일 전송이 진행 중이라면 강제 취소(cancel) 요청
            - Worker 스레드 중지(stop) 호출
            - 이름이 없으면 모든 활성 연결 종료

        Args:
            name (Optional[str]): 종료할 연결 이름. None이면 모든 연결 종료.
        """
        if name:
            # 파일 전송 중이라면 취소 요청
            ...
            # 모든 포트 닫기 (KeyError 방지를 위해 리스트 복사본으로 순회)
    ...
    def send_broadcast_data(self, data: bytes) -> None:
        """
        브로드캐스트가 허용된 모든 활성 연결로 데이터를 전송합니다.

        Logic:
            - 활성 Worker 목록 순회
            - 각 Worker의 broadcast_enabled() 상태 확인
            - True인 경우 데이터 전송
    ...
    def send_data_to_connection(self, name: str, data: bytes) -> bool:
        """
        특정 연결의 Worker 큐에 데이터를 추가하여 전송합니다.
    ...
    def set_dtr(self, state: bool) -> None:
        """
        모든 활성 포트의 DTR(Data Terminal Ready) 신호를 설정합니다.
    ...
    def set_rts(self, state: bool) -> None:
        """
        모든 활성 포트의 RTS(Request To Send) 신호를 설정합니다.
