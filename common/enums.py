...
class ConnectionEventState(Enum):
    """
    PortConnectionEvent.state 값에 사용하는 연결 생명주기 상태입니다.

    Attributes:
        OPENED: 연결이 정상적으로 열린 상태 이벤트.
        CLOSED: 연결이 정상적으로 닫힌 상태 이벤트.
    """
    OPENED = "opened"
    CLOSED = "closed"
...
class MacroStepType(Enum):
    """
    MacroStepEvent.type 값에 사용하는 단계 이벤트 타입입니다.

    Attributes:
        STARTED: 매크로 단계 실행 시작.
        COMPLETED: 매크로 단계 실행 완료.
    """
    STARTED = "started"
    COMPLETED = "completed"
...
class LogLevel(Enum):
    """
    시스템 로그와 오류 컨텍스트에서 사용하는 공통 로그 레벨입니다.
...
class LanguageType(Enum):
    """
    설정 파일에 저장되는 지원 언어 코드입니다.

    Attributes:
        ENGLISH: 영어 리소스 코드(en).
        KOREAN: 한국어 리소스 코드(ko).
    """
    ENGLISH = "en"
    KOREAN = "ko"
