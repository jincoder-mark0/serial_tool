...
# 종료 시 TX 드레인을 기다리며 UI 이벤트를 흘리는 주기.
#
# WHY: 종료 경로는 유실을 막기 위해 드레인 완료를 기다려야 하는데, QThread.wait()로
#      기다리면 이벤트 루프가 멈춰 창이 굳는다. 짧은 주기로 끊어 기다리며 그 사이
#      그리기 이벤트를 흘리면 "멈춘 것"이 아니라 "내보내는 중"으로 보인다.
SHUTDOWN_FLUSH_POLL_MS: int = 50
