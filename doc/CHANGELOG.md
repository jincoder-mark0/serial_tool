...
### 크래시 다이얼로그가 모달 위에 모달로 쌓이지 않게 (2026-09-06)

`GlobalErrorHandler._show_error_dialog`는 `show_error_signal` 슬롯이고 내부에서
`QMessageBox.exec_()`로 **중첩 이벤트 루프**를 돕니다. 그 루프가 큐에 밀려 있던 다음
오류를 처리하면 첫 다이얼로그 **안에서** 두 번째가 열렸습니다(실측 중첩 깊이 2).

타이머 슬롯처럼 반복 실행되는 곳에서 예외가 나면 다이얼로그가 무한정 쌓이고, 하나를
닫으면 다음이 나와 앱을 닫을 수 없습니다. 스로틀도 중복 제거도 없었습니다.

이 프로젝트는 **같은 위험을 이미 알고 고쳤습니다** — S-082가
`MainWindow.show_error_message`를 `QTimer.singleShot(0, ...)`로 지연시켰고
`tests/test_modal_not_reentrant.py`가 그 성질을 지킵니다. 크래시 경로만 빠져 있었습니다.

- 다이얼로그를 한 번에 하나만 엽니다. 열려 있는 동안 온 오류는 세어만 두고, 닫힐 때
  억눌린 건수를 경고로 남깁니다. 개별 traceback은 `_process_exception`이 이미
  CRITICAL로 기록하므로 유실이 아닙니다.
- 표시 로직을 `_render_error_dialog()`로 분리하고, 가드는 `finally`로 반드시
  풀립니다 — 렌더 실패 한 번으로 이후 모든 오류가 영구 침묵하면 막으려던 것보다
  나쁩니다.

**S-082와 같은 방식(지연)을 쓰지 않은 이유**를 적어 둡니다. 지연하면 이벤트 루프가
돌기 전에 난 오류는 **아무것도 보이지 않습니다.** `main.py`는 `app.exec_()` 이전에
설정·리소스·부트스트랩을 모두 수행하므로, 거기서 죽으면 사용자가 가장 필요한 순간에
침묵합니다. 실측으로 확인한 뒤 이 경로는 동기 표시를 유지하고 재진입만 막기로
했습니다.

회귀 테스트 5건을 `tests/test_crash_dialog_reentrancy.py`에 추가했습니다. 가드를
제거하면 2건이, S-082처럼 지연으로 바꾸면 5건 전부가 실패합니다 — 두 방향 모두
고정합니다.

검증 결과는 `808 passed`(3회 반복 동일), Ruff 0건, language/task-board gate Green입니다.
