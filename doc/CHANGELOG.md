...
### 매크로 엔진 고도화 및 성능 최적화 (2025-12-18)
...
- **실행 컨텍스트 추적 (Execution Context Tracking)**
  - **부분 실행 지원**: `(RowIndex, Entry)` 튜플 구조를 도입하여, 리스트 정렬이나 필터링 상태와 무관하게 원본 행을 정확히 추적하고 UI와 동기화합니다.
  - **로컬 에코 연동**: 매크로를 통해 전송되는 데이터도 전역 설정에 따라 로그 뷰(Data Log)에 표시되도록 기능을 보완했습니다.

#### 리팩토링 (Refactoring)

- **초기화 프로세스 최적화 (Initialization Optimization)**
  - **지연 로딩 (Lazy Initialization)**: `LanguageManager` 초기화 시 리소스 경로가 없으면 파일 스캔을 지연시켜 애플리케이션 초기 구동 속도를 개선하고 중복 로딩을 방지했습니다.
  - **실행 순서 재정립**: `main.py`의 실행 흐름을 '설정 로드 → 매니저 초기화 → 언어 적용 → UI 생성 → 테마 적용' 순으로 변경하여 초기 구동 시 테마 깜빡임 현상을 제거했습니다.
...
- **안정성 및 예외 처리 (Stability & Safety)**
  ...
  - **브로드캐스트 예외**: 브로드캐스트 모드에서 모든 포트의 연결이 끊어졌을 때 실행이 계속되는 문제(Ghost Run)를 해결했습니다.

- **버그 수정 (Bug Fixes)**
  - **색상 코드 보정**: `ColorManager`에서 설정 파일(`color_rules.json`) 로드 시 `#` 접두사가 누락된 HEX 코드를 자동으로 보정하도록 수정했습니다.
