...
전 페이지 감사(`tools/ui_audit.py`: 다크/라이트 × 한글/영문 × 기본/최소 창
= 8조합 × 13페이지, 스텝형 페이지는 장마다)에서 나온 규칙이다. **`tests/unit/
test_ui_guidelines.py`가 기계적으로 강제**한다 — 어기면 테스트가 실패한다.
새 화면을 만들 때 이 문서를 먼저 읽는다.

규칙은 대부분 **실제로 깨진 것**에서 나왔다. 각 항목의 "실측/사용자 지적"
표시가 그 근거다 - 근거가 사라지면 규칙도 다시 볼 수 있다.
...
- 기준 창 크기는 **실제 값**을 쓴다: 시작
  `UI_DEFAULT_WINDOW_W`×`UI_DEFAULT_WINDOW_H`(1280×720), 최소
  `UI_MIN_WINDOW_W`×`UI_MIN_WINDOW_H`(940×560) — PyDracula 템플릿의
  `resize`/`setMinimumSize`와 일치하며 테스트가 이를 고정한다. 두 크기 모두에서 잘림·겹침이 없어야 한다.
...
- 키 규약: 한 화면에만 쓰는 문구는 `page.<페이지>.<요소>`, 공용 위젯은
  `widget.<위젯>.<요소>`, 여러 화면이 공유하는 낱말은 `common.<낱말>`.
  화면을 넘나드는 **주제**는 그 주제 이름을 접두로 쓴다 (`prepare.`,
  `prepare_stage.`, `board_monitor.`, `device.`, `menu.`, `submenu.`,
  `nav.`, `update.`) - 실제 사전이 그렇게 되어 있다.
- ko/en 사전(`resources/lang/*.json`)에 **양쪽 모두** 값을 넣는다
  (`test_language_dictionaries_have_same_keys`가 누락을 잡는다).
...
- **라벨끼리도 겹치면 안 된다.** 칸 폭을 항목 간격에 맞추고 넘치면 말줄임,
  전체 문구는 툴팁으로 남긴다 (실측 2026-08-22: 스테퍼가 라벨 칸을 고정
  120px로 그려 최소 창 8스텝에서 이름이 서로 겹쳤다 - 감사는 QPainter로
  그린 글자를 보지 못하므로 **위젯 단위 테스트**로 막는다).
- 회전 텍스트(세로 축 제목)는 폭을 확보해 글자가 겹치지 않게 한다.

## 7-1. 확인 팝업

- 팝업은 `widgets.dialogs.build_confirm_box`/`ask`로 만든다. Qt 표준
  버튼(OK/Cancel)은 앱 언어를 따르지 않아 한국어 실행에서도 영문으로 뜬다.
- **Enter = 확인, Esc = 취소.** 이 팝업은 방금 누른 동작을 되묻는 자리이지
  새 동작을 제안하는 자리가 아니다 - Enter가 취소로 가면 눌러도 아무 일이
  없어 멈춘 것처럼 보인다 (사용자 지적 2026-08-21).
- 크기는 QMessageBox에 맡긴다. 내부 라벨에 `min-width`를 주면 **아이콘
  라벨까지** 늘어나 아이콘과 글자 사이가 벌어진다 (실측).
- worker 스레드에서 사람을 기다릴 때는 모달 `exec()`를 쓰지 않는다 -
  `docs/testing.md`의 "worker 스레드 경계" 참조.
