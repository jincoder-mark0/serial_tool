...
- **View**:
    ...
    - `view/widgets/rx_log.py`: Newline 처리, ColorManager 적용, 버그 수정
    - `view/widgets/port_settings.py`: 메서드 복원 및 설정 조회 로직 추가
    - `view/widgets/manual_ctrl.py`: Local Echo, 히스토리 추가
...
- **Presenter**:
    - `presenter/main_presenter.py`: 상태바 업데이트, 단축키 처리, 종료 로직, Local Echo 구현
    - `presenter/port_presenter.py`: 설정 조회 로직 변경, 에러 핸들링 개선
- **Model/Core**:
    - `model/port_controller.py`: data_sent 시그널, open_port 리팩토링
