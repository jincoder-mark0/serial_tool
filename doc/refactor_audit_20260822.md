...
### 후속 처리 완료 (2026-08-22, 사용자 지시 "후속후보 진행, 불일치는 통일")

| 태스크 | 대상 | 커밋 |
|---|---|---|
| S-052 | 로그 위젯 제어 흐름 **Presenter 권위로 통일** | f0317e4 |
| S-053 | ThemeManager(833줄) → FontManager + ThemeResourceLoader + 오케스트레이터 | f0317e4 |
| S-054 | ColorManager(511줄) → Qt 비의존 ColorRuleRepository + 어댑터 | f0317e4 |
| S-036 | **고정폭 폰트 설정 미반영**(실측 확정) + 언어팩 잔여 4건 | a41fc52 |
| S-055 | 시스템 로그 저장 실제 구현(구독자 0건이던 것) | 87014dd |
| S-056 | 크래시 다이얼로그 계층 역전 정리 + **계층 의존 자동 검사** | 87014dd |
| S-057 | ruff 잔여 44건 정리 → **0건 달성·CI 게이트 전환** | 71bd1ee |
| S-059 | **종료 시 RX 로거 미정리 유실**(stop_all 호출처 0건) | 71bd1ee |
| S-058 | MainPresenter 분해(설정·종료·포맷 결정) | 6389d59 |

후속 처리 중 **새 결함 3건이 추가로 드러났다**: 시스템 로그 저장 미배선(S-055),
`stop_all()` 미호출로 종료 시 데이터 유실(S-059 — S-045에서 고친 안전장치가 정작
호출되지 않고 있었다), DataLog 브로드캐스트 초기값 불일치(S-051).

테스트 **134 → 367**. ruff 0건이 CI 게이트로 강제된다.

### 남은 것 (근거 있는 보류·후속)

- **패킷 뷰 무스로틀**(C-6 인접): 패킷 1건마다 즉시 UI 반영(30ms 버퍼 미적용).
  RX raw와 성격이 달라 즉시 결함은 아니다. **측정 없는 최적화 금지 원칙**(S-007)에 따라
  고속 패킷 실측 전까지 보류 — 착수 시 첫 스텝은 LOOPBACK 고속 주입 측정이다.
- **`PortScanWorker` 종료 시 미정리**(S-059 조사에서 발견): 온디맨드 초단명 스레드이고
  파일·데이터 핸들이 없어 최악이 스레드 leak 수준. 저위험으로 목록만 남김.
- **로그 위젯 필터 판정 로직 중복**(S-055 보고): "보이는 것=저장되는 것"을 구현하며
  `QSmartListView`의 필터 판정을 `system_log.py`가 재현했다. 한쪽만 바뀌면 어긋난다 —
  장기적으로 `QSmartListView`에 공개 판정 메서드를 두는 편이 낫다.
- **S-007 성능 최적화**: 실측상 병목 없음으로 보류(재개 조건 기록됨).
- **S-009 플러그인 / S-010 가상 시리얼 / S-015 SPI·I2C**: 요구 확정·사용자 개입 대기.
