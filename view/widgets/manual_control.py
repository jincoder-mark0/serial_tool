...
class ManualControlWidget(QWidget):
    ...
    def init_ui(self) -> None:
        ...
        self.option_group = QGroupBox(language_manager.get_text("manual_grp_control"))
        option_layout = QGridLayout()
        option_layout.setContentsMargins(2, 2, 2, 2) # 내부 여백 최소화
        option_layout.setSpacing(5)

        self.hex_mode_check = QCheckBox(language_manager.get_text("manual_chk_hex"))
        self.hex_mode_check.setToolTip(language_manager.get_text("manual_chk_hex_tooltip"))

        self.enter_check = QCheckBox(language_manager.get_text("manual_chk_enter"))
        self.enter_check.setChecked(True)
        self.enter_check.setToolTip(language_manager.get_text("manual_chk_enter_tooltip"))

        self.clear_btn = QPushButton(language_manager.get_text("global_btn_clear"))
        self.clear_btn.setToolTip(language_manager.get_text("global_btn_clear"))
        self.clear_btn.clicked.connect(self.clear_info_requested.emit)

        self.save_log_btn = QPushButton(language_manager.get_text("global_btn_save"))
        self.save_log_btn.setToolTip(language_manager.get_text("global_btn_save"))
        self.save_log_btn.clicked.connect(self.on_save_log_clicked)

        # 흐름 제어 (Flow Control - RTS/DTR)
        self.rts_check = QCheckBox(language_manager.get_text("manual_chk_rts"))
        self.rts_check.setToolTip(language_manager.get_text("manual_chk_rts_tooltip"))
        self.dtr_check = QCheckBox(language_manager.get_text("manual_chk_dtr"))
        self.dtr_check.setToolTip(language_manager.get_text("manual_chk_dtr_tooltip"))
        ...
        self.send_btn = QPushButton(language_manager.get_text("manual_btn_send"))
        ...
        self.file_group = QGroupBox(language_manager.get_text("manual_grp_file"))
        file_layout = QHBoxLayout()
        file_layout.setContentsMargins(2, 2, 2, 2)
        file_layout.setSpacing(5)

        self.file_path_label = QLabel(language_manager.get_text("manual_lbl_no_file"))
        self.file_path_label.setStyleSheet("color: gray; border: 1px solid #555; padding: 2px; border-radius: 2px;")

        self.select_file_btn = QPushButton(language_manager.get_text("manual_btn_select_file"))
        self.select_file_btn.clicked.connect(self.on_select_file_clicked)

        self.send_file_btn = QPushButton(language_manager.get_text("manual_btn_send_file"))
    ...
    def retranslate_ui(self) -> None:
        ...
        self.option_group.setTitle(language_manager.get_text("manual_grp_control"))
        self.hex_mode_check.setText(language_manager.get_text("manual_chk_hex"))
        self.enter_check.setText(language_manager.get_text("manual_chk_enter"))
        self.clear_btn.setText(language_manager.get_text("global_btn_clear"))
        self.save_log_btn.setText(language_manager.get_text("global_btn_save"))
        ...
        self.file_group.setTitle(language_manager.get_text("manual_grp_file"))
        # 파일이 선택되지 않은 상태인지 확인
        if "No file selected" in self.file_path_label.text() or \
           "파일이 선택되지 않음" in self.file_path_label.text():
              self.file_path_label.setText(language_manager.get_text("manual_lbl_no_file"))

        self.select_file_btn.setText(language_manager.get_text("manual_btn_select_file"))
        self.send_file_btn.setText(language_manager.get_text("manual_btn_send_file"))
    ...
    def on_select_file_clicked(self) -> None:
        """파일 선택 버튼 클릭 시 호출됩니다."""
        path, _ = QFileDialog.getOpenFileName(self, language_manager.get_text("manual_dialog_select_file"))
        if path:
    ...
    def on_send_file_clicked(self) -> None:
        ...
        if path and path != language_manager.get_text("manual_lbl_no_file"):
            self.send_file_requested.emit(path)

    def on_save_log_clicked(self) -> None:
        """로그 저장 버튼 클릭 시 호출됩니다."""
        filter_str = f"{language_manager.get_text('dialog_file_filter_txt')} (*.txt);;{language_manager.get_text('dialog_file_filter_all')} (*)"
